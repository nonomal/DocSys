...
public class ToolPromptBuilder {
    ...
    public static String buildSystemPrompt(List<ToolDefinition> tools) {
        ...
        sb.append("## How to use tools\n")
          ...
          .append("3. To call a tool, output exactly one block. The content inside the block MUST be ")
          .append("valid JSON with keys \"name\" and \"arguments\". Do NOT use <invoke> or <parameter> ")
          .append("XML tags, and do NOT wrap in <tool_calls> (plural) — only this exact singular JSON form:\n")
          .append("   <tool_call>{\"name\":\"tool_name\",\"arguments\":{\"param\":\"value\"}}</tool_call>\n")
          .append("   (start with <tool_call>, end with </tool_call>, exactly one JSON object inside)\n")
