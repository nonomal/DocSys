#!/bin/bash
# Test: upload_doc

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUN_SCRIPT="$SCRIPT_DIR/../scripts/run.sh"

# Test 1: Missing args shows usage
if bash "$RUN_SCRIPT" 2>&1 | grep -q "Usage:"; then
  echo "PASS: Missing args shows usage"
else
  echo "FAIL: Should show usage without args"
  exit 1
fi

# Test 2: Non-existent file shows error
if bash "$RUN_SCRIPT" "/nonexistent/file.txt" 1 2>&1 | grep -qi "not found"; then
  echo "PASS: Non-existent file shows error"
fi

echo '{"success":true,"docId":789,"name":"report.pdf","vid":1}' | jq .
echo "PASS: Mock response valid JSON"

echo "All tests passed."
